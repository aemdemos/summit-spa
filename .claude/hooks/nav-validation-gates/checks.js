...
export function checkMobileRegisters(workspaceRoot) {
  ...
  if (!fs.existsSync(path.join(workspaceRoot, MOBILE_SCHEMA_REGISTER))) {
    errors.push('mobile-schema-register.json does NOT exist. Run detect-mobile-structure.js first, then extract migrated-mobile-structural-summary.json (same shape as mobile-structure-detection.json), then: node .claude/skills/excat-navigation-orchestrator/scripts/compare-mobile-structural-schema.js .../mobile/mobile-structure-detection.json .../mobile/migrated-mobile-structural-summary.json --output-register=.../mobile/mobile-schema-register.json');
...
export function checkMobileDimensionalGate(workspaceRoot) {
  ...
  if (!report || report.passed !== true) {
    ...
    errors.push(
      `[MOBILE] mobile-dimensional-gate report exists but passed is not true: ${summary}. Fix CSS (e.g. .nav-list and .nav-item width: 100%) and re-run: node .claude/skills/excat-navigation-orchestrator/scripts/mobile-dimensional-gate.js --url=<migrated-url> --validation-dir=blocks/header/navigation-validation. Do not proceed until the gate passes.`,
    );
    remediation.push(
