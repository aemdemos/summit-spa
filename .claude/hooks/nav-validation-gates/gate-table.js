    ...
    check: (ctx) => {
      ...
      if (migratedMmExists && !behaviorRegExists && hasMegamenu(wr)) {
        if (ctx.basename !== 'migrated-megamenu-mapping.json') {
          blocks.push('migrated-megamenu-mapping.json exists but compare-megamenu-behavior.js has NOT been run.\n  → Run NOW: node .claude/skills/excat-navigation-orchestrator/scripts/compare-megamenu-behavior.js blocks/header/navigation-validation/megamenu-mapping.json blocks/header/navigation-validation/migrated-megamenu-mapping.json --output=blocks/header/navigation-validation/megamenu-behavior-register.json');
        }
      }
  ...
  // Gate 7, 8, 12: mobile-style-register.json
  {
    ...
    check: (ctx) => {
      ...
      if (dimGateResult.errors.length > 0) {
        const msg = `🚫 [Nav Gate] [MOBILE] mobile-style-register.json is BLOCKED until mobile-dimensional-gate passes.\n\n${dimGateResult.errors.join('\n\n')}\n\nRun: node .claude/skills/excat-navigation-orchestrator/scripts/mobile-dimensional-gate.js --url=<migrated-url> --validation-dir=blocks/header/navigation-validation (viewport 375×812). Fix any failed checks (e.g. .nav-list and .nav-item width: 100%) and re-run until exit 0.`;
        return { pass: false, message: msg };
      }
      return { pass: true };
    },
