...
function handleStop(hookInput) {
  ...
  if (fs.existsSync(phase1Path) && !fs.existsSync(rowDetectionMarkerPath)) {
    allErrors.push('phase-1-row-detection.json exists but detect-header-rows.js has NOT been run. Run: node .claude/skills/excat-navigation-orchestrator/scripts/detect-header-rows.js --url=<source-url> — programmatic row detection is mandatory; never set rowCount from screenshot alone.');
  ...
  if (navFilePath) {
    ...
    if (!fs.existsSync(navValidatedPath)) {
      ...
      allErrors.push(`${path.relative(workspaceRoot, navFilePath)} exists but validate-nav-content.js has NOT been run. Run: node .claude/skills/excat-navigation-orchestrator/scripts/validate-nav-content.js content/${navBasename} blocks/header/navigation-validation — validates image existence and size > 0.`);
  ...
  const phase4PathForStop = path.join(workspaceRoot, VALIDATION_DIR, 'phase-4-mobile.json');
  let styleResult = { errors: [], remediation: [] };
  if (fs.existsSync(phase4PathForStop)) {
  ...
  allRemediation = [...(styleResult.remediation || []), ...(schemaResult.remediation || []), ...(megamenuResult.remediation || [])];
  ...
  if (allErrors.length > 0) {
    ...
    try {
      ...
      if (fs.existsSync(logDir)) {
        const entry = `[${new Date().toISOString()}] [HOOK:PREMATURE-COMPLETION] Stop attempted with ${allErrors.length} unresolved issue(s). LLM should have run pre-completion-check.js first.\n`;
