...
2. **Run the row detection script (REQUIRED — gate blocks otherwise):**
   ```
   node .claude/skills/excat-navigation-orchestrator/scripts/detect-header-rows.js --url=<source-url> [--validation-dir=blocks/header/navigation-validation]
   ```
   The script uses Playwright and auto-detects `.claude/skills/excat-navigation-orchestrator/scripts/playwright-browsers` if present. If "Executable doesn't exist", run `PLAYWRIGHT_BROWSERS_PATH=$PWD/.claude/skills/excat-navigation-orchestrator/scripts/playwright-browsers npx playwright install chromium` from the project root. The script navigates, runs `page.evaluate()` (getBoundingClientRect, getComputedStyle), and writes `phase-1-row-detection.json` + `.row-detection-complete` marker. **Do NOT write phase-1 manually** — the script produces it.
...
3. **Implement desktop only:**
   ...
   - **Image download (CRITICAL — images are missing every time, this is the #1 failure):** Before writing nav.plain.html, you MUST complete these sub-steps in order:
     ...
     5. After writing nav.plain.html, **MANDATORY — run the validation script:**
        ```
        node .claude/skills/excat-navigation-orchestrator/scripts/validate-nav-content.js content/nav.plain.html blocks/header/navigation-validation
        ```
     ...
     5b. **MANDATORY — run the image audit script** (compares expected vs actual header images; blocks if gap):
        ```
        node .claude/skills/excat-navigation-orchestrator/scripts/audit-header-images.js content/nav.plain.html blocks/header/navigation-validation
        ```
...
4b. **Row elements behavior validation (required for all desktop headers) — Step 5a.** Every row, every element — logo, nav links, CTA, search, locale selector, hamburger, icons — must have hover and click validated. Not just megamenu; not just main nav headings.
   ...
   - **Compare and write register:** Run:
     ```
     node .claude/skills/excat-navigation-orchestrator/scripts/compare-row-elements-behavior.js \
...
7. **Pre-confirmation gate + Customer Confirmation (desktop structural + behavioral only):** Before asking the customer, verify:
   - [ ] Run `node .claude/skills/excat-navigation-orchestrator/scripts/validate-nav-content.js content/nav.plain.html blocks/header/navigation-validation` — exit code MUST be 0.
   - [ ] Run `node .claude/skills/excat-navigation-orchestrator/scripts/audit-header-images.js content/nav.plain.html blocks/header/navigation-validation` — exit code MUST be 0 (expected vs actual header images; no gap).
...
Validate all sub-agent output with `node .claude/skills/excat-navigation-orchestrator/scripts/validate-output.js <output.json> <schema.json>`. Exit non-zero = do not proceed. Schemas: `references/desktop-navigation-agent-schema.json`, `references/mobile-navigation-agent-schema.json`, `references/megamenu-schema.json`, `references/validation-agent-schema.json`.
...
**Key scripts:** `.claude/skills/excat-navigation-orchestrator/scripts/validate-output.js`, `validate-nav-content.js`, `audit-header-images.js`, `detect-header-rows.js`, `detect-mobile-structure.js`, `compare-megamenu-behavior.js`, `compare-row-elements-behavior.js`, `compare-header-appearance.js`, `compare-structural-schema.js`, `compare-mobile-structural-schema.js`, `mobile-dimensional-gate.js`, `pre-completion-check.js` (all under `.claude/skills/excat-navigation-orchestrator/scripts/`).
